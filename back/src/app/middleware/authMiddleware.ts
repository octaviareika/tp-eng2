...
export const isAuthenticated = (
  ...
  if (!req.session || !req.session.usuario) {
    return res
      .status(401)
      .json({ message: "Não autorizado: Sessão não encontrada ou inválida." });
  }
...
export const isAluno = (req: Request, res: Response, next: NextFunction) => {
  if (
    ...
    return res
      .status(403)
      .json({
        message: "Acesso proibido: Apenas alunos podem realizar esta ação.",
      });
  }
  next();
};
...
export const isFuncionario = (
  ...
  if (
    ...
    return res
      ...
      .json({
        message:
          "Acesso proibido: Apenas funcionários podem realizar esta ação.",
      });
  }
  next();
};
